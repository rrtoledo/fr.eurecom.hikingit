package fr.eurecom.hikingit.edittrack;
...
public class EditMapFragment extends Fragment implements OnMapClickListener,
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View rootView = inflater.inflate(R.layout.editmap_fragment, container,
				false);
		...
		//Retrieving data from the listView
		...
		case R.id.item3:
			if (ButtonEnableMarkClicked == false) {
				...
				if (vectorLoc.isEmpty()) {
					...
					Log.w("fr.eurecom.hikingit",
							"Sending data from EditMapFragment "
									+ data.toString());
					mListener.OnFragmentClick(2, data);
					Log.w("fr.eurecom.hikingit",
							"Coords sent from EditMapFragment");
				}
			} else
	...
	private void fillData(Uri uri){
		String[] projection = { TrackTable.COLUMN_COORDS,
				TrackTable.COLUMN_NBCOORDS};
		...
		else Log.w("fr.eurecom.hikingit","EMF Cursor is null");
	}
	
	private void drawTrack(){
		Log.w("fr.eurecom.hikingit","DrawTrack, vectorLoc "+vectorLoc.toString());
		...
		for(int i=0;i<vectorLoc.size();i++){
			...
			if(vectorLoc.size()>1 && i<vectorLoc.size()-1){
				Polyline linetemp = googleMap
					.addPolyline(new PolylineOptions()
							.add(vectorLoc.get(i),
									vectorLoc.get(i+1)).width(3)
							.color(Color.RED));
				polylines.add(i, linetemp);
			}
		}
	}
